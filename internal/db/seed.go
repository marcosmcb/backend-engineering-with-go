...
func Seed(store store.Storage) {
	...
	posts := generatePosts(2_000, users)
	...
	cms := generateComments(5_000, users, posts)
...
func generateUsers(num int) []*store.User {
	...
	for i := 0; i < num; i++ {
		username := usernames[i%len(usernames)] + fmt.Sprintf("%d", rand.Intn(len(usernames)*len(usernames)*len(usernames)))
		users[i] = &store.User{
			Username: username,
